...
let centimes_of : int -> int -> int =
  fun (rp:int) -> fun (fr:int) -> fr * 100 + rp
...
let rev_t (l: 'a list) : 'a list =
  let rec aux (l: 'a list) (accum: 'a list) : 'a list =
    begin match l with
      | [] -> accum
      | h::t -> aux t (h::accum)
    end
...
let rec insert (x:'a) (l:'a list) : 'a list =
  begin match l with
    | [] -> [x]
    | h::t -> if x <= h then (if x < h then x::h::t else h::t) else h::insert x t
  end
...
let rec interpret (c:ctxt) (e:exp) : int64 =
  begin match e with
    ...
    | Neg e1 -> Int64.sub Int64.zero (interpret c e1)
  end
